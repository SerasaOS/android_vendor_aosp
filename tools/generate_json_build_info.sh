...
if [ "$1" ]; then
    ...
    DEVICE=$(echo $TARGET_PRODUCT | sed 's/morbid_//g')
    if [ -f $file_path ]; then
        ...
        if [[ $file_name == *"official"* ]] || [[ $FORCE_JSON == 1 ]]; then
            ...
            build_type=$(grep ro\.morbid\.build\.type ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            base_version=$(grep ro\.morbid\.base\.version ./out/target/product/$DEVICE/system/build.prop | cut -d= -f2);
            link="https://sourceforge.net/projects/project-morbid/files/${DEVICE}/unsub/${file_name}/download"
            echo "{" > $file_path.json
