...
func init() {
	android.RegisterModuleType("morbid_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = morbidExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = morbidExpandVariables(ctx, dep_file)
	...
	cmd := morbidExpandVariables(ctx, String(g.properties.Cmd))
