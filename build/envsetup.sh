function __print_morbid_functions_help() {
cat <<EOF
Additional Morbid functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- morbidgerrit:   A Git wrapper that fetches/pushes patch from/to Morbid Gerrit Review.
- morbidrebase:   Rebase a Gerrit change and push it again.
- morbidremote:   Add git remote for Morbid Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for Morbid Github.
...
function breakfast()
{
    target=$1
    local variant=$2
    local aosp_target_release=$(cat ${ANDROID_BUILD_TOP}/vendor/morbid/vars/aosp_target_release 2>/dev/null)
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the morbid model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch morbid_$target-$aosp_target_release-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/morbid-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.morbid.device | grep -q "$MORBID_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $MORBID_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function morbidremote()
{
    ...
    git remote rm morbid 2> /dev/null
    ...
    local MORBID_USER=$(git config --get review.review.morbid.org.username)
    if [ -z "$MORBID_USER" ]
    then
        git remote add morbid ssh://review.morbid.org:29418/$PFX$PROJECT
    else
        git remote add morbid ssh://$MORBID_USER@review.morbid.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'morbid' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/Ngabluu/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.morbid.device | grep -q "$MORBID_BUILD");
    then
...
function makerecipe() {
    ...
    then
        pwd
        morbidremote
        git push morbid HEAD:refs/heads/'$1'
    fi
    '
}

function morbidgerrit() {
    ...
    local user=`git config --get review.review.morbid.org.username`
...
function morbidrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "Morbid Gerrit Rebase Usage: "
        echo "      morbidrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.morbid.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function dopush()
{
    ...
    if (adb shell getprop ro.morbid.device | grep -q "$MORBID_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    python3 $T/vendor/morbid/build/tools/repopick.py $@
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $MORBID_FIXUP_COMMON_OUT ]; then
